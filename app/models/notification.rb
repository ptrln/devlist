class Notification < ActiveRecord::Base
  ...
  def self.latest_for_user(user)
    ...
    return Notification.where("(notifiable_type = 'User' AND notifiable_id IN (#{following_user_ids.join(',')})) OR (notifiable_type = 'UserProject' AND notifiable_id IN (#{following_project_ids.join(',')}))").all
